...
class AccountManager:
    ...
    def add_account(self, user_login, user_password):
        """Add new account to database"""
        with Session(self.engine) as session:
            user_account = Account(user_login=user_login, user_password=user_password)
            session.add_all([user_account])
            session.commit()

    def load_account(self, user_login, user_password):
        """
        ...
        try:
            with Session(self.engine) as session:
                [user_id] = [account.id for account in session.query(Account).filter(Account.user_login==user_login)]
                [user_login] = [account.user_login for account in session.query(Account).filter(Account.user_login==user_login)]
                [user_password] = [account.user_password for account in session.query(Account).filter(Account.user_login==user_login)]
